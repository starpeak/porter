namespace :porter do  
  namespace :db do
    ...
    task :prepare_namespace do
      ...
      puts 'prepage porter table_name_prefix'
      ActiveRecord::Base.configurations = Rails::Application.config.database_configuration
      
      class ActiveRecord::Base
        self.table_name_prefix = "#{::ActiveRecord::Base.table_name_prefix}#{Porter::config.table_name_prefix}"
      end
    end
    
    task :remove_namespace do
      puts 'remove porter table_name_prefix'
      
      class ActiveRecord::Base
        self.table_name_prefix = "#{::ActiveRecord::Base.table_name_prefix[Porter::config.table_name_prefix.length,1000]}"
      end
    end
    ...
    namespace :migrate do
      ...
      task :redo => [:environment, :prepare_namespace] do
        if ENV["VERSION"]
          Rake::Task["porter:db:migrate:down"].invoke
          Rake::Task["porter:db:migrate:up"].invoke
        else
          Rake::Task["porter:db:rollback"].invoke
          Rake::Task["porter:db:migrate"].invoke
        end
      end
    ...
    task :abort_if_pending_migrations => [:environment, :prepare_namespace] do
      if defined? ActiveRecord
        ...
        if pending_migrations.any?
          ...
          abort %{Run "rake porter:db:migrate" to update your database then try again.}
        end
      end
    end
  end
...
task :'db:migrate' => [:'porter:db:migrate', :'porter:db:remove_namespace']