  ...
  module Generators
    class Base < Rails::Generators::Base #:nodoc:
      def self.source_root
        @_porter_source_root ||= File.expand_path(File.join(File.dirname(__FILE__), 'porter', generator_name, 'templates'))
      end
 
      def self.banner
        "#{$0} porter:#{generator_name} #{self.arguments.map{ |a| a.usage }.join(' ')} [options]"
      end
    end
